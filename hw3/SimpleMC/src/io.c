...
// Read in parameters from file. for local
void parse_parameters_loc(Parameters *parameters_loc)
{
  ...
  while((s = fgets(line, sizeof(line), fp)) != NULL){
    ...
    // (LOC) Number of particles
    else if(strcmp(s, "particles") == 0){
      long long n_particles = atoll(strtok(NULL, "=\n"));
      if(n_particles < 1)
        print_error("Number of particles must be greater than 0");
      assert(n_particles % nprocs == 0); // divisibility
			n_particles =  n_particles / nprocs;
			parameters_loc->n_particles = n_particles;
    }

    // Number of batches
    else if(strcmp(s, "batches") == 0){
      parameters_loc->n_batches = atoi(strtok(NULL, "=\n"));
    }

    // Number of generations
    else if(strcmp(s, "generations") == 0){
      parameters_loc->n_generations = atoi(strtok(NULL, "=\n"));
    }

    // Number of active batches
    else if(strcmp(s, "active") == 0){
      parameters_loc->n_active = atoi(strtok(NULL, "=\n"));
    }

    // Number of nuclides in material
    else if(strcmp(s, "nuclides") == 0){
      parameters_loc->n_nuclides = atoi(strtok(NULL, "=\n"));
    }
    ...
    // Whether to tally
      ...
      s = strtok(NULL, "=\n");
      if(strcasecmp(s, "true") == 0)
        parameters_loc->tally = TRUE;
      else if(strcasecmp(s, "false") == 0)
        parameters_loc->tally = FALSE;
      else
        print_error("Invalid option for parameter 'tally': must be 'true' or 'false'");
    }

    // (LOC) Number of bins in each dimension
    else if(strcmp(s, "bins") == 0){
      int n_bins = atoi(strtok(NULL, "=\n"));
			assert(n_bins % nprocs == 0); // divisibility
			n_bins = n_bins / nprocs;	
			parameters_loc->n_bins = n_bins;
		}

    // RNG seed
    else if(strcmp(s, "seed") == 0){
      parameters_loc->seed = atol(strtok(NULL, "=\n"));
    }

    // Average number of fission neutrons produced
    else if(strcmp(s, "nu") == 0){
      parameters_loc->nu = atof(strtok(NULL, "=\n"));
    }

    // Fission macro xs
    else if(strcmp(s, "xs_f") == 0){
      parameters_loc->xs_f = atof(strtok(NULL, "=\n"));
    }

    // Absorption macro xs
    else if(strcmp(s, "xs_a") == 0){
      parameters_loc->xs_a = atof(strtok(NULL, "=\n"));
    }

    // Scattering macro xs
    else if(strcmp(s, "xs_s") == 0){
      parameters_loc->xs_s = atof(strtok(NULL, "=\n"));
    }

    // (LOC) Geometry size in x
    else if(strcmp(s, "x") == 0){
      int gx = atof(strtok(NULL, "=\n"));
			assert(gx % nprocs == 0); // divisibility
			gx = gx / nprocs;
			parameters_loc->gx = gx;
		}

    // (LOC) Geometry size in y
    else if(strcmp(s, "y") == 0){
      int gy = atof(strtok(NULL, "=\n"));
			assert(gy % nprocs == 0); // divisibility
			gy = gy / nprocs;
			parameters_loc->gy = gy;
    }

    // (LOC) Geometry size in z
    else if(strcmp(s, "z") == 0){
      int gz = atof(strtok(NULL, "=\n"));
			assert(gz % nprocs == 0); // divisibility
			gz = gz / nprocs;
			parameters_loc->gz = gz;
    }
    ...
    // (Default FALSE) Whether to output tally
    else if(strcmp(s, "write_tally") == 0){
			parameters_loc->write_tally = FALSE;
			/*
				 s = strtok(NULL, "=\n");
				 if(strcasecmp(s, "true") == 0)
				 parameters_loc->write_tally = TRUE;
				 else if(strcasecmp(s, "false") == 0)
				 parameters_loc->write_tally = FALSE;
				 else
				 print_error("Invalid option for parameter 'write_tally': must be 'true' or 'false'");
		...
		// (Default FALSE) Whether to output keff
		else if(strcmp(s, "write_keff") == 0){
			parameters_loc->write_keff = FALSE;
			/*
				 s = strtok(NULL, "=\n");
				 if(strcasecmp(s, "true") == 0)
				 parameters_loc->write_keff = TRUE;
				 else if(strcasecmp(s, "false") == 0)
				 parameters_loc->write_keff = FALSE;
				 else
				 print_error("Invalid option for parameter 'write_keff': must be 'true' or 'false'");
		...
		// Path to write tallies to
			...
			s = strtok(NULL, "=\n");
			parameters_loc->tally_file = malloc(strlen(s)*sizeof(char)+1);
			strcpy(parameters_loc->tally_file, s);
		}

		// Path to write keff to
		else if(strcmp(s, "keff_file") == 0){
			s = strtok(NULL, "=\n");
			parameters_loc->keff_file = malloc(strlen(s)*sizeof(char)+1);
			strcpy(parameters_loc->keff_file, s);
		}
...
void write_tally_parallel(int mype, int n_workers, Parameters *parameters, 
		Tally *tally_loc, char *filename)
{
	int i, j, k;
	int i_loc, j_loc;	
	int i_lvl, j_lvl, k_lvl;
	...
	for (i = 0; i < parameters->n_bins; i++)
	{
		for (j = 0; j < parameters->n_bins; j++)
		{
			i_loc = i % tally_loc->n;
			j_loc = j % tally_loc->n;
			...
			mype_start = i_lvl + n_workers_axis*j_lvl + n_workers_axis*n_workers_axis*k_lvl;
			if (mype_start >= n2)
			{
				printf("%d, %d, %d, %d\n", i_lvl, j_lvl, tally_loc->n, mype_start);
			}
			assert(mype_start < n2);

			for (k = mype_start; k < n_workers; k += n2) 
			{
				write_tally_loc(mype, tally_loc, i_loc, j_loc, k, filename);
		...
		int mype_dest, char *filename)
{
	if (mype == mype_dest)
	{
		...
		for (int k = 0; k < tally_loc->n; k++)
		{
			fprintf(fp, "%e ", tally_loc->flux[i_loc + tally_loc->n*j_loc + tally_loc->n*tally_loc->n*k]);
		}
